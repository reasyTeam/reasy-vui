<template>
  <div class="vui-picker-wrapper border-bottom">
    <div class="vui-form-row" :class="{ disabled: isDisabled }" @click="show">
    ...
    <transition name="vui-fade">
      <div class="vui-pick-mask" v-show="visible" @click="hide"></div>
    </transition>
    <transition name="vui-slide">
      <div class="vui-picker" v-show="visible">
        <div class="vui-picker-head border-bottom">
          <label
            class="vui-picker-button vui-picker-cancel"
            @click="cancel"
            :style="{ color: cancelColor }"
          >
            {{ cancelText }}</label
          >
          <h4 class="vui-picker-title" :style="{ color: titleColor }">
            {{ title }}
          </h4>
          <label
            class="vui-picker-button vui-picker-submit"
            @click="confirm"
            :style="{ color: confirmColor }"
            >{{ confirmText }}</label
          >
        </div>
        <div class="vui-picker-panel">
          <div class="vui-picker-top border-bottom"></div>
          <div class="vui-picker-bottom border-top"></div>
          <div class="vui-picker-wheel-wrapper" ref="wheelWrapper">
            <div
              class="vui-picker-wheel"
              v-for="(data, index) in pickerData"
              :key="index"
            >
              <ul class="vui-picker-wheel-scroll">
                <li
                  v-for="item in data"
                  :key="item.value"
                  :class="{ 'vui-picker-wheel-disabled-item': item.disabled }"
                  class="vui-picker-wheel-item"
                >
                  {{ item.title }}
                </li>
              </ul>
      ...
      if (!wheels[i]) {
        wheels[i] = new BScroll(wheelWrapper.children[i], {
          wheel: {
            ...
            wheelWrapperClass: "vui-picker-wheel-scroll",
            wheelItemClass: "vui-picker-wheel-item"
          },
          probeType: 3
        });
