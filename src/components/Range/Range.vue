<template>
  <div class="vui-range-wrapper" :class="[`border-${border}`]">
    <div class="vui-form-row" :class="{ disabled: isDisabled }">
      <label class="vui-form-label" v-if="label">{{ label }}</label>
      <div class="vui-range vui-form-value">
        <label class="vui-range-min">
          <slot name="start">{{ min }} </slot>
        </label>
        <div class="vui-range-content" ref="content">
          <div
            class="vui-range-runway"
            :style="{ 'border-top-width': barHeight + 'px' }"
          ></div>
          <div
            class="vui-range-progress"
            :style="{ width: progress + '%', height: barHeight + 'px' }"
          ></div>
          <div
            class="vui-range-thumb"
            ref="thumb"
            :style="{ left: progress + '%' }"
          ></div>
        </div>
        <label class="vui-range-max">
